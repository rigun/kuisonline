    ...
    <div class="columns m-t-10">
      ...
      <div class="card" v-if="soal != null && kesempatan > 0">
          <div class="card-content">
              <h1>{{soal.quiz}}</h1>
              <v-radio-group v-model="data.option_id">
                  <v-radio  v-for="opsi in soal.option" :key="opsi.id" :value="opsi.id" :label="opsi.option"></v-radio>
              </v-radio-group>
          </div>
          <div class="card-action" style="text-align: right; width: 100%">
              <v-btn color="blue" dark @click.prevent="sendData()" :loading="load">Jawab</v-btn>
          </div>
      </div>
